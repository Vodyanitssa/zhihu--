...
private fun convertElementToBlock(
    ...
    "p" -> {
        ...
        if (element.childNodeSize() == 0) {
            ...
            element.textWithOnlyWhitespace() &&
            element
                .child(0)
                .tagName() == "br"
        ) {
...
private fun extractInlineChildren(
    ...
    return childNodes.flatMapIndexed { index, child ->
        if (child is TextNode && child.text().isBlank()) {
            if (childNodes.take(index).any { it.hasInlineContent() } &&
                childNodes
                    .drop(index + 1)
                    .any { it.hasInlineContent() }
            ) {
                listOf(Text(" "))
            } else {
                emptyList()
    ...
    is Element -> when (node.tagName().lowercase()) {
        ...
        "sup" -> {
            if (node.attr("data-draft-type") == "reference") {
                ...
                parsingDocument!!.footnoteDefinitions[node.attr("data-numero")] =
                    FootnoteDefinition(index.toString(), index).apply {
                        appendChild(
                            Paragraph().apply {
...
private fun MarkdownNode.appendMarkdownInline(out: StringBuilder) {
    when (this) {
        ...
        is Superscript ->
            out
                .append("<sup>")
                .also { children.forEach { it.appendMarkdownInline(out) } }
                .append("</sup>")
