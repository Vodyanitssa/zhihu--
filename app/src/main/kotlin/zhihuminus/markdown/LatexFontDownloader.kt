...
suspend fun downloadLatexFonts(context: Context, client: HttpClient): DownloadedFonts = withContext(Dispatchers.IO) {
    ...
    if (!lmFile.exists()) {
        ...
        if (!lmFile.exists()) {
            throw lastError
                ?: RuntimeException("Failed to download Latin Modern Math from all mirrors")
        }
    }
    ...
    val caligraphicRegular =
        fontFromFile(File(dir, "KaTeX_Caligraphic-Regular.ttf"), FontWeight.Normal, FontStyle.Normal)
